...
class encryption{
    private:
        ...
        void Matrix_to_String(vector<vector<uint8_t>>& matrix,int i) {
            string result;
        ...
        void printSBox(uint8_t S[16][16]) {
            for (int i = 0; i < 16; ++i) {
                for (int j = 0; j < 16; ++j) {
                    cout << "0x" << hex << setw(2) << setfill('0') << static_cast<int>(S[i][j]) << " ";
                }
                cout << std::endl;
            }
}
...
class decryption{
    ...
    decryption(encryption& enc){
     ...
     void Process(encryption& enc){
        cout<<"\nInverse S BOX \n-------------------------------------------------------------------------------\n";
        enc.printSBox(Inv_S_BOX);
        //Add roundkey , Round 0
            ...
            // cout<<"Round key : "<<endl;
                ...
                enc.Matrix_to_String(enc.State,1);
                return;
    }
